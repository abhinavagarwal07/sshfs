...
def test_sshd_kill_no_reconnect(tmpdir, capfd):
    ...
    # Start secondary sshd (try with sudo first, then without)
    sshd_proc = None
    for cmd in [
        ["sudo", sshd_path, "-f", sshd_config_path, "-D"],
        [sshd_path, "-f", sshd_config_path, "-D"],
    ]:
        sshd_proc = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(0.5)
        if sshd_proc.poll() is None:
            break
        sshd_proc = None
